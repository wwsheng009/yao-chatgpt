...
func (r *Request) Send(method string, data interface{}) *Response {
	...
	if r.query != nil && len(r.query) > 0 {
		urlstr = fmt.Sprintf("%s?%s", urlstr, r.query.Encode())
	}
	...
	if http_proxy == "" {
		http_proxy = os.Getenv("ALL_PROXY")
	}
	if http_proxy == "" {
		http_proxy = os.Getenv("all_proxy")
	}
	if http_proxy != "" {
		httpProxy, err := url.Parse(http_proxy)
		if err != nil {
	...
	if strings.HasPrefix(r.url, "https://") {
		...
		if https_proxy != "" {
			httpsProxy, err := url.Parse(https_proxy)
			if err != nil {
				return ResponseError(400, err.Error())
			}
